...
# Reference
# The idea of architecture was learned from https://nextjournal.com/gkoehler/pytorch-mnist
class Net(nn.Module):
    def __init__(self):
...
def decaptcha( filenames ):
	...
	# Initializing numChars(numpy array) and codes(list)
	numChars = np.array( [ 0 for i in range(l) ] )
	codes = []

	for index in range(l):
		# read image from filepaths
		image = cv2.imread(filenames[index])
		hsv_image = cv2.cvtColor(image,cv2.COLOR_BGR2HSV)

		# remove obfuscating lines and convert to gray scale image
		_, gray_image = cv2.threshold(hsv_image[:,:,2], 203, 255, cv2.THRESH_BINARY)
		gray_image = np.asarray(gray_image)

		# find columns containing at least one black pixel
		val = np.unique(np.where(gray_image==0)[1])
		length_val = len(val)
		start, end = val[0], -1
		
		# segmented array stores the segmentation of the given image into separate characters
		segmented = []
		...
		for i in range(length_val):
			...
			if end != -1:
				if end - start >= 20:
					# extracting only the character from the image
					resize = gray_image[:, start:end]

					# align the character symmetrically w.r.t horizontal
					resize = horizontal_sym(end-start, resize)

					# find rows containing at least one black pixel
					hor_zeros = np.unique(np.where(resize==0)[0])
					resize = resize[hor_zeros[0]:hor_zeros[-1]+1, :]

					# align the character symmetrically w.r.t vertical
		...
		# number of characters in the given image
